...
public class CborDecoderArrayTest extends CborDecoderTestBase<Object> {

    public CborDecoderArrayTest(Object output, int[] encodedInput) {
    ...
    public static Iterable<Object[]> getParameters() {
        ...
        return Arrays.asList( //
            ...
            new Object[] { Arrays.asList(1L, 2L, 3L, 4L, 5L), new int[] { 0x9f, 0x01, 0x02, 0x03, 0x04, 0x05, 0xFF } } // 6
            );
        // @formatter:on
    }
    ...
    public void test() throws IOException {
        ...
        if (m_expectedOutput instanceof Object[]) {
            ...
            List<?> actual = (List<?>) readGenericItem();
            assertEquals(expected.length, actual.size());

            for (int i = 0; i < expected.length; i++) {
                assertElementEquals(expected[i], actual.get(i));
            }
        } else {
            List<?> expected = (List<?>) m_expectedOutput;

            List<?> actual = (List<?>) readGenericItem();
            assertEquals(expected.size(), actual.size());

            for (int i = 0; i < expected.size(); i++) {
                assertElementEquals(expected.get(i), actual.get(i));
            }
        }
    }
}
